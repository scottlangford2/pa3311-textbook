...
(kahneman1974heuristics). Feynman’s warning complements this idea—analysts must rely on
...
variation, a foundational concept in statistical inference (casella2002statistical,
...
Group comparisons are ubiquitous in public administration:
    ...
    - Do two neighborhoods experience different average 311 completion times? (austin311open)
    - Are EMS response times slower in rural than urban districts? (dshsEMS)
    ...
    - Do ACS estimates of income differ across two census tracts? (uscensusACS)


But descriptive differences alone cannot answer these questions. Sampling variation (Chapter
*chap:inference*) ensures that two sample means will differ even when population means are equal
(ross2014probability). Independent *t*-tests formalize the comparison by quantifying
...
service delivery (gelman2007data), while Freedman warns that naïve comparisons may produce
misleading conclusions if uncertainty is ignored (freedman2007statisticalmodels).
...
$$
H_0: \mu_A = \mu_B \qquad vs.} \qquad H_1: \mu_A \ne \mu_B.
$$


The logic mirrors the structure of classical hypothesis testing described by Casella and Berger
(casella2002statistical): compare an observed statistic to its sampling distribution under the
...
Administrative examples include:
    ...
    - comparing service performance across demographic groups (with care for representativeness
          (gaoDataQuality, sampson2012great)),
...
were generated (freedman2007statisticalmodels). Administrative datasets rarely arise from
random sampling (einav2014datarev), so analysts must interpret results within institutional
context.


## The Formula and Its Intuition

Let $X}_A$ and $X}_B$ be the sample means of Groups A and B. Their difference is:
...
expected variation (freedman2007statisticalmodels).
...
rates (sampson2012great, austin311open).
...
$$
H_0: \mu_{EP}} = \mu_{Peer}} \qquad H_1: \mu_{EP}} \neq \mu_{Peer}}
$$
...
$$
SE(\Delta) = s_1^2}{n_1} + s_2^2}{n_2}} = 4.8^2}{12} + 5.2^2}{15}} = 23.04}{12} + 27.04}{15}} = 1.92 + 1.80} = 3.72} = 1.93
$$
...
$$
t = x}_1 - x}_2}{SE(\Delta)} = 42.3 - 48.1}{1.93} = -5.8}{1.93} = -3.01
$$
...
$$
d = |x}_1 - x}_2|}{s_{pooled}}} = 5.8}{5.0} = 1.16
$$
...
(einav2014datarev, freedman2007statisticalmodels, gaoDataQuality).
...
$$
d = X}_A - X}_B}{s_{pooled}}}.
$$


Interpretation of effect size must consider the specific operational context in which the difference matters. Two minutes of EMS response time may appear small in a statistical summary but can be the difference between life and death in a cardiac emergency. Small average differences in service delivery may mask large subgroup disparities---an overall gap of half a day in 311 completion times might conceal a two-day gap for specific request types in specific neighborhoods. Measurement error in both administrative and survey systems can inflate or deflate effect sizes in ways that are difficult to detect without careful data auditing (uscensusACS, gaoDataQuality). Finally, institutional context shapes interpretation: a moderate effect size in a setting characterized by systemic neighborhood inequality (sampson2012great) may carry far greater policy significance than a large effect size in a setting where the baseline is already equitable.
...
Cleveland and Tukey emphasize that graphical analysis clarifies practical significance by
displaying effect magnitude and distributional shape (clevelandGraphing, tukeyEDA).
...
As Manski warns, analysts often overstate the certainty of small differences
(manski2019policy).
...
ACS and BLS estimates require attention to margins of error and design effects (uscensusACS,
blsHandbook).
...
(einav2014datarev, gaoDataQuality).
...
## Practice and Application
    ...
    - Compare 311 completion times across two neighborhoods using Welch’s *t*-test.
          Use graphical diagnostics to examine skewness (austin311open, tukeyEDA).

    - Evaluate whether EMS response times differ between two districts.
          Assess both statistical and operational significance (dshsEMS, ombPerfGuide).

    - Using ACS median income estimates for two census tracts, compute the independent
          *t*-test while incorporating margins of error (uscensusACS).
    ...
    - Compare processing times before and after a staffing reform (independent samples).
          ...
          (casella2002statistical, freedman2007statisticalmodels).
...
Chapter~*chap:pairedttests* introduces paired *t*-tests, which compare two measurements taken on the *same* units---before and after a policy change, or across two time points---isolating within-unit change.