...
same units. A city manager may want to know whether average 311 completion times improved after a new workflow reform (City of Austin open data), or whether an EMS district responded faster after staffing increases (Texas DSHS EMS data). Inspectors and their supervisors ask whether inspection durations decreased after digital recordkeeping was introduced. Planners track whether census tracts experienced income growth from one ACS cycle to the next (U.S. Census ACS), and community development officials examine whether residents in a neighborhood saw improved service delivery after targeted investment. In every case, the analyst is comparing two measurements of the same unit---not two separate groups of units.
...
emphasized repeatedly in statistical literature (Gelman & Hill 2007; Freedman 2007). A paired design eliminates variation between individuals by using each unit as its own control, dramatically increasing sensitivity to detect change, especially in small samples. When units vary widely at baseline---as neighborhoods, districts, and agencies almost always do---the gains from pairing can be substantial.
...
In an independent *t*-test (Chapter 6), Group A and Group B
contain separate units. In a paired *t*-test, each observation in Period 1 has a natural partner
in Period 2:


$$\text{Unit}_i^{(1)}\leftrightarrow\text{Unit}_i^{(2)}$$
...
respect the hierarchical and longitudinal structure of their data (Gelman & Hill 2007).
...
> **Returning to the Case:** In the Proposition B case, $D_i$ represents the change in homeless count for census tract $i$: $D_i = \text{count}_{i,\text{after}} - \text{count}_{i,\text{before}}$. If the ban reduced visible homelessness, we would expect $\bar{D} < 0$. The paired design is essential here because tracts differ enormously in baseline counts---a tract near downtown Austin might count 50 individuals while a suburban tract counts 3. Comparing aggregate before-and-after totals (an unpaired approach) would be dominated by the high-count tracts. The paired difference isolates within-tract change, giving equal analytical weight to each geographic unit.
...
| Tract | Before | After | $D_i$ (after − before) |
|---|---|---|---|
| 1 | 50 | 37 | −13 |
| 2 | 12 | 10 | −2 |
| 3 | 30 | 21 | −9 |
...
$$\bar{D}=\frac{(-13)+(-2)+(-9)+3+(-13)+(-1)+(-11)+3}{8}=\frac{-43}{8}=-5.375$$


**Step 2: Compute the standard deviation of differences.**

$$s_D=\sqrt{\frac{\sum(D_i-\bar{D})^2}{n-1}}=6.48$$


**Step 3: Compute the $t$-statistic.**

$$t=\frac{\bar{D}}{s_D/\sqrt{n}}=\frac{-5.375}{6.48/\sqrt{8}}=\frac{-5.375}{2.29}=-2.35$$


**Step 4: Evaluate.**
With $df = 7$, the critical value for a two-tailed test at $\alpha = 0.05$ is approximately $\pm 2.36$. Our $t = -2.35$ falls just short of the threshold ($p \approx 0.051$). The result is borderline---suggestive of a decline but not conclusive at conventional significance levels. In Excel, use `=T.TEST(before_range, after_range, 2, 1)` for a two-tailed paired test.

**Interpretation.**
The average tract-level count declined by about 5.4 individuals, but substantial variation across tracts (some increased, most decreased) means we cannot rule out chance at the 5% level.


*Figure: A histogram of the eight tract-level differences would show most tracts declining, with two small increases, clustered around the mean change of about −5.*
...
$$
\bar{D} = \text{mean difference}, \qquad
...
$$\text{Var}(D_i)<\text{Var}(X_{i,\text{before}})+\text{Var}(X_{i,\text{after}})$$
...
a workflow redesign (City of Austin open data).
...
error to quantify uncertainty (U.S. Census ACS).
...
Following Gelman and Hill’s advice on hierarchical and longitudinal data (Gelman & Hill 2007),
...
reflect shifts in reporting rather than operational improvement.
...
Overstating the certainty of a result is a common error.  
...
Chapter 8 introduces regression analysis, the framework for modeling relationships among multiple variables simultaneously.